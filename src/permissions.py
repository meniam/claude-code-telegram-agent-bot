...
Special-cased: `AskUserQuestion`. Built-in to Claude Code, but useless in
headless SDK because it has no UI. We intercept the call, render every
question as a Telegram inline keyboard, collect the user's pick(s) into a
plain-text summary and ship it back to Claude as the tool's denial reason —
the model treats the message as the tool's response and adapts.
"""
...
class TelegramPermissionGate:
    ...
    async def can_use_tool(
        ...
        if tool_name == "AskUserQuestion":
            return await self._handle_ask_user_question(chat_id, tool_input)
        t = self._t
    ...
    async def _handle_ask_user_question(
        ...
        self._aq_aborted.discard(chat_id)
        try:
            for qidx, q in enumerate(questions):
                if chat_id in self._aq_aborted:
                ...
                if not isinstance(q, dict):
                    continue
                try:
                    answers = await self._ask_one(chat_id, qidx, len(questions), q)
                except asyncio.TimeoutError:
                    try:
                        await self._bot.send_message(
        ...
        return PermissionResultDeny(message=self._format_aq_answers(collected))
    ...
    async def _ask_one(
        ...
        def build_kb() -> InlineKeyboardMarkup:
            session = self._aq.get(request_id)
        ...
        self._aq[request_id] = _AQSession(
            fut=fut,
            options=options,
            multi=multi,
            chat_id=chat_id,
        ...
        try:
            picked_idx = await asyncio.wait_for(fut, timeout=self._timeout)
        finally:
            self._aq.pop(request_id, None)
            await self._delete_prompt(chat_id, sent.message_id)
    ...
    async def handle_aq_callback(self, callback: CallbackQuery) -> None:
        ...
        if action == "done":
            if not session.multi:
                await callback.answer()
                return
            session.fut.set_result(sorted(session.selected))
